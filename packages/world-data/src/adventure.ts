...
export type AdventureRegion={
  id:string;name:string;size:RegionSize;severity:RegionSeverity;complexity:number;domain:string;
  contradiction:string;goals:RegionGoal[];requirements:ThresholdRequirement[];rewards:Record<'BARE'|'COMPETENT'|'MASTERY',RegionReward[]>;
...
const ARCHETYPES=[
  ['bellweather','Bellweather','SMALL','FORGIVING','OWNERSHIP','Something is owned before anyone agrees it is.'],
...
const DIMENSION_BY_DOMAIN:Record<string,UnderstandingDimension>={
  MATTER:'manipulation',SPACE:'navigation',TIME:'causality',MONEY:'economy',LIFE:'systems',DEATH:'anomaly_reasoning',IDENTITY:'social',KNOWLEDGE:'knowledge',OWNERSHIP:'ownership',CAUSALITY:'causality',SOCIAL:'social',ENTROPY:'systems'
};
const REWARD_POOL:RegionReward[]=[
  {kind:'PROPERTY',key:'room',label:'A room that remains yours when you leave it.'},
  {kind:'CAPABILITY',key:'known_destination_move',label:'Known places may be addressed by name.'},
  {kind:'INTERFACE',key:'memory_confidence',label:'Memory Field gains confidence marks.'},
  {kind:'RESOURCE',key:'regional_stipend',label:'A modest regional resource allowance.'},
  {kind:'PRIVILEGE',key:'records_access',label:'Access to local records.'},
  {kind:'PROPERTY',key:'workbench',label:'A persistent workbench.'},
  {kind:'CAPABILITY',key:'awkward_carry',label:'Awkward objects become carryable.'},
  {kind:'INTERFACE',key:'relationship_marks',label:'Known NPC relationships become visible.'},
  {kind:'PRIVILEGE',key:'after_hours',label:'One institutional facility recognises you after hours.'},
  {kind:'RESOURCE',key:'route_token',label:'A reusable regional travel token.'},
];
...
function rewards(seed:string,regionId:string,offset:number,count:number){
  const start=hashString(`${seed}:${regionId}:${offset}`)%REWARD_POOL.length;
  return Array.from({length:count},(_,i)=>REWARD_POOL[(start+i*3)%REWARD_POOL.length]!);
}

function makeRequirements(domain:string,complexity:number,size:RegionSize,severity:RegionSeverity):ThresholdRequirement[]{
  const primary=DIMENSION_BY_DOMAIN[domain]??'systems';
  const scale=SIZE_MULT[size]*SEVERITY_MULT[severity]*(1+complexity*.08);
  const dims:[UnderstandingDimension,number][]=[[primary,1],['perception',.72],['navigation',.62],['breadth',.42]];
  return dims.map(([dimension,weight])=>{
    const bare=Math.round(5*scale*weight);
    return {dimension,bare,competent:Math.round(bare*1.65+2),mastery:Math.round(bare*2.55+5)};
  });
}

function makeGoals(regionId:string,size:RegionSize,severity:RegionSeverity,index:number):RegionGoal[]{
  const scale=SIZE_MULT[size]*SEVERITY_MULT[severity];
  return [
...
export function buildAdventure(seedInput:number|string,directions:readonly DirectionLike[]){
  ...
  const count=8+(hashString(seed)%9);
  const selected=[ARCHETYPES[0]!,...Array.from({length:count-1},(_,i)=>ARCHETYPES[1+seeded(seed,i,ARCHETYPES.length-1)]!)];
  const unique=[] as typeof ARCHETYPES[number][]; const seen=new Set<string>();
  for(const row of [...selected,...ARCHETYPES]){if(!seen.has(row[0])){seen.add(row[0]);unique.push(row);}if(unique.length>=count)break;}
  const regions:AdventureRegion[]=unique.map((row,index)=>{
    const [id,name,sizeRaw,severityRaw,domain,contradiction]=row;
    const size=sizeRaw as RegionSize,severity=severityRaw as RegionSeverity;
    const complexity=1+seeded(seed,index+41,9);
    return {id,name,size,severity,complexity,domain,contradiction,goals:makeGoals(id,size,severity,index),requirements:makeRequirements(domain,complexity,size,severity),
      rewards:{BARE:rewards(seed,id,0,2),COMPETENT:rewards(seed,id,1,3),MASTERY:rewards(seed,id,2,4)},exits:{}};
  });
  if(!directions.length)return {seed,regions,startRegionId:'bellweather'};
  for(let i=0;i<regions.length-1;i+=1){
    const direction=directions[i%directions.length]!;
    regions[i]!.exits[direction.key]=regions[i+1]!.id;
    if(i>0&&i%3===0){const branch=regions[(i+2)%regions.length]!;const branchDirection=directions[(i+1)%directions.length]!;regions[i]!.exits[branchDirection.key]=branch.id;}
  }
  return {seed,regions,startRegionId:'bellweather'};
}

export function evaluateRegionThreshold(region:AdventureRegion,profile:Partial<UnderstandingProfile>,state:AdventureState):ThresholdGrade{
  const meets=(grade:'bare'|'competent'|'mastery')=>region.requirements.every(requirement=>(profile[requirement.dimension]??0)>=requirement[grade]);
  const goalRatio=region.goals.length?state.completedGoals/region.goals.length:1;
  if(meets('mastery')&&goalRatio>=1&&state.visitedLocations>=Math.max(3,Math.round(SIZE_MULT[region.size]*4)))return 'MASTERY';
  if(meets('competent')&&goalRatio>=.6)return 'COMPETENT';
  if(meets('bare')&&goalRatio>=.25&&state.visitedLocations>=2)return 'BARE';
  return 'FAIL';
}

export function insufficientThresholdHint(region:AdventureRegion,profile:Partial<UnderstandingProfile>){
  const deficits=region.requirements.map(requirement=>({dimension:requirement.dimension,gap:requirement.bare-(profile[requirement.dimension]??0)})).filter(row=>row.gap>0).sort((a,b)=>b.gap-a.gap);
  const worst=deficits[0];
  if(!worst)return 'The region objects for reasons it has declined to document.';
  const hints:Record<UnderstandingDimension,string>={
    perception:'You have passed several things without establishing what they were.',navigation:'The boundary appears unconvinced by your relationship with getting places.',manipulation:'You have not demonstrated enough control over ordinary objects.',social:'Other people remain largely theoretical in your record.',economy:'You appear unfamiliar with value changing hands.',production:'The region expects evidence that you can make something become something else.',knowledge:'You have observations. The boundary was hoping for understanding.',institutional:'Paperwork has not yet learned to fear you.',ownership:'Possession and ownership are not yet sufficiently distinct in your behaviour.',causality:'The region expects a firmer relationship with cause and consequence.',systems:'You have operated parts without showing that you recognise a whole.',anomaly_reasoning:'Contradiction remains more surprising to you than useful.',breadth:'Your understanding is impressively narrow.',depth:'You have touched many ideas without seriously inconveniencing any of them.'
  };
  return hints[worst.dimension];
}
