...
export type RegionAssessment={
  regionId:string;name:string;condition:string;assessment:'INSUFFICIENT'|'SUFFICIENT'|'STRONG'|'COMPLETE';grade:ThresholdGrade;
  completedGoals:Array<{id:string;label:string;complete:boolean;progress:number;target:number}>;hint:string|null;rewards:RegionReward[];nextRegions:Array<{directionKey:string;shape:string;label:string;regionId:string}>;
};

export async function assessCurrentRegion(playerId:string):Promise<RegionAssessment>{
  const progress=await getPlayerProgress(playerId);const region=adventure.regions.find(row=>row.id===progress.currentRegion)??adventure.regions[0]!;
  ...
  const [conceptRow]=await db.select({count:sql<number>`count(*)`}).from(playerConcepts).where(eq(playerConcepts.playerId,playerId));
  const [anomalyRow]=await db.select({count:sql<number>`count(*)`}).from(anomalyClaimsV2).where(eq(anomalyClaimsV2.playerId,playerId));
  const visited=Number(visitedRow?.count??1)||1,questions=Number(questionRow?.count??0),concepts=Number(conceptRow?.count??0),anomalies=Number(anomalyRow?.count??0);
  ...
  if(!stored||rank[grade]>rank[stored.grade as ThresholdGrade]){
    const newlyEligible=grade==='FAIL'?[]:region.rewards[grade];const byKey=new Map([...existingRewards,...newlyEligible].map(reward=>[`${reward.kind}:${reward.key}`,reward]));rewards=[...byKey.values()];
  ...
  const assessment=grade==='FAIL'?'INSUFFICIENT':grade==='BARE'?'SUFFICIENT':grade==='COMPETENT'?'STRONG':'COMPLETE';
  ...
  return {regionId:region.id,name:region.name,condition:region.contradiction,assessment,grade,completedGoals:goals.map(goal=>({id:goal.id,label:goal.label,complete:goal.complete,progress:goal.progress,target:goal.target})),hint:grade==='FAIL'?insufficientThresholdHint(region,progress.understanding):null,rewards,nextRegions};
}
