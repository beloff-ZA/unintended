...
import { giveItem, guardDrop, resolvePlayerAtSamePlace, stealItem, tradeItems } from './custody.js';

const give=/^(?:give|hand|return)\s+(?:the\s+)?(.+?)\s+to\s+(.+)$/i;
...
export async function handleCustodyCommand(playerId:string,text:string):Promise<string[]|null>{
 let match=text.match(drop);if(match){const guarded=await guardDrop(playerId,match[1]!);if(guarded)return guarded;return null;}
 match=text.match(give);if(match){const recipient=await resolvePlayerAtSamePlace(playerId,match[2]!);if(!recipient)return ['The intended recipient is not presently close enough for ownership to become their problem.'];const result=await giveItem(playerId,recipient.id,match[1]!,'GIVE');if(!result.ok)return result.reason==='NOT_HELD'?['You do not currently possess that.']:resistedLines(result.item?.name);if(!result.item)return ['Ownership changed without leaving behind a useful description.'];return [`You give the ${result.item.name} to ${recipient.name}.`,`Ownership changes without requiring a speech.`];}
 match=text.match(lend);if(match){const recipient=await resolvePlayerAtSamePlace(playerId,match[2]!);if(!recipient)return ['The intended borrower is not presently close enough to disappoint you.'];const result=await giveItem(playerId,recipient.id,match[1]!,'LEND');if(!result.ok)return result.reason==='NOT_HELD'?['You do not currently possess that.']:resistedLines(result.item?.name);if(!result.item)return ['The loan exists, but the Server has misplaced the noun.'];return [`You lend the ${result.item.name} to ${recipient.name}.`,`The Server has recorded an expectation of return. It has not recorded optimism.`];}
 match=text.match(steal);if(match){const victim=await resolvePlayerAtSamePlace(playerId,match[2]!);if(!victim)return ['Nobody by that description is sufficiently near and present to be robbed.'];const result=await stealItem(playerId,victim.id,match[1]!);if(!result.ok)return result.reason==='NOT_FOUND'?['The intended property is not available to be stolen from that person.']:resistedLines(result.item?.name);if(!result.item)return ['Something changed hands, but not cleanly enough to celebrate.'];return [`The ${result.item.name} changes hands without permission.`,`The Server records this as custody, not innocence.`];}
