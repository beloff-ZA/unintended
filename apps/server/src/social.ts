...
const FACETS = ['Bellweather','Lowwater','Terminus','The Works','Verdant','The Archive'] as const;
const TASKS = [
...
export async function socialReach(playerId:string){
  const origin=originForPlayer(playerId);const links=await worldLinks();const adjacent=new Map<string,Set<string>>();
  for(const map of SOCIAL_MAPS)adjacent.set(map.id,new Set());
  for(const link of links){adjacent.get(link.from)?.add(link.to);adjacent.get(link.to)?.add(link.from);}
  const reached=new Set<string>([origin.id]),queue=[origin.id];
  while(queue.length){const current=queue.shift()!;for(const next of adjacent.get(current)??[]){if(reached.has(next))continue;reached.add(next);queue.push(next);}}
  const maps=SOCIAL_MAPS.filter(map=>reached.has(map.id));const visibleLinks=links.filter(link=>reached.has(link.from)&&reached.has(link.to));
...
export async function maybeLinkMapForProgress(playerId:string,regionId:string,grade:string){
  ...
  return {from:reach.origin,to:target};
}

function relationshipLevel(familiarity:number,trust:number,established:boolean){
  const score=familiarity*.65+trust*.35;if(established&&score>=82)return 'ENTRENCHED';if(score>=64)return 'TRUSTED';if(score>=44)return 'KNOWN';if(score>=24)return 'FAMILIAR';if(score>=8)return 'RECOGNISES';return 'STRANGER';
}
...
export async function returnRecap(playerId:string,relationships:RelationshipView[]){
  const [last]=await db.select({createdAt:worldEvents.createdAt}).from(worldEvents).where(and(eq(worldEvents.actorId,playerId),sql`${worldEvents.type} <> 'PLAYER_ORIGIN_ASSIGNED'`)).orderBy(desc(worldEvents.createdAt)).limit(1);
  if(!last?.createdAt)return [] as string[];const absentMs=Date.now()-last.createdAt.getTime();if(absentMs<2*60*60*1000)return [];
  const hours=Math.floor(absentMs/3600000);const history=await db.select().from(importantHistory).where(sql`${importantHistory.createdAt} > ${last.createdAt}`).orderBy(desc(importantHistory.createdAt)).limit(3);
  const lines=[`You were absent for approximately ${hours} hour${hours===1?'':'s'}.`];for(const item of history.reverse())lines.push(item.summary);
  const cooled=relationships.filter(row=>row.needsAttention&&row.lastInteractionAt).slice(0,2);for(const row of cooled)lines.push(`${row.npcName} has had time to become less certain about your continued relevance.`);
  if(lines.length===1)lines.push('Nothing sufficiently important happened. This is probably temporary.');return lines;
}
