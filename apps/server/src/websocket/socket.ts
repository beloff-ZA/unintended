...
export function attachWebSocket(app:FastifyInstance){
 ...
 wss.on('connection',async(ws:WebSocket)=>{const playerId=(ws as any).playerId as string,set=clients.get(playerId)??new Set<WebSocket>();set.add(ws);clients.set(playerId,set);await redis.set(`presence:${playerId}`,'online','EX',120);broadcast({type:'PRESENCE',text:'Someone arrived.',at:new Date().toISOString()},ws);
  ws.on('message',async raw=>{try{
   ...
   if(relationshipMatch){
    const actor=await repo.getActor(playerId),target=await repo.findVisibleEntity(actor.locationId,relationshipMatch[1]!);
    if(target?.kind!=='NPC')return send(ws,{type:'OUTPUT',lines:['The intended social maintenance is clear. The intended person is not currently available.'],at:new Date().toISOString()});
    const maintained=await repo.maintainRelationship(playerId,target.id);
    if(!maintained)return send(ws,{type:'OUTPUT',lines:[`${target.name} is not in a position to participate in this relationship at present.`],at:new Date().toISOString()});
    const lines=[`${target.name} allows you to be useful in a small, unheroic way.`,maintained.task,`Relationship: ${maintained.level}.`];
    if(maintained.established)lines.push('Routine maintenance is becoming less necessary. History is beginning to do some of the remembering for you.');
    send(ws,{type:'OUTPUT',lines,at:new Date().toISOString()});return;
   }
   let result=await engine.execute(playerId,parsed.data.text),interpretedFrom:string|undefined;
   if(result.semantic?.kind==='UNKNOWN'){
    try{const rewritten=await ai.reinterpretKnown(playerId,parsed.data.text);if(rewritten&&rewritten.toLowerCase()!==parsed.data.text.trim().toLowerCase()){interpretedFrom=rewritten;result=await engine.execute(playerId,rewritten);}}catch{}
   }
   let lines=result.lines;if(interpretedFrom)lines=[`The Server interpreted that as: ${interpretedFrom}`,...lines];
   else{try{const augmented=await ai.augment(playerId,parsed.data.text,result);if(augmented?.length)lines=augmented;}catch{}}
   send(ws,{type:'OUTPUT',lines,at:new Date().toISOString()});
   if(result.events.some(event=>['PLAYER_MOVED','ITEM_TAKEN','ITEM_DROPPED','DOOR_OPENED','PLAYER_DISCOVERED_ANOMALY','THRESHOLD_PASSED','MAP_LINKED'].includes(event.type)))broadcast({type:'PRESENCE',text:'Something changed nearby.',at:new Date().toISOString()},ws);
