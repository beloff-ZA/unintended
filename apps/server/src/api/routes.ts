...
export async function apiRoutes(app:FastifyInstance){
 ...
 app.get('/api/me',async(req,reply)=>{const id=await getSession(req.cookies.session);if(!id)return reply.code(401).send({error:'Unauthenticated'});const state=await buildPlayerState(id);if(!state)return reply.code(404).send({error:'Player missing'});return state;});
 ...
 if(process.env.DEV_AUTH==='true')app.post('/api/dev/login/:slot',async(req,reply)=>{const slot=Number((req.params as any).slot)||1;const email=`dev${slot}@unintended.local`;let [u]=await db.select().from(users).where(eq(users.email,email));if(!u)[u]=await db.insert(users).values({email}).returning();let [c]=await db.select().from(characters).where(eq(characters.userId,u.id));if(!c)[c]=await db.insert(characters).values({userId:u.id,name:`Tester ${slot}`}).returning();const token=await createSession(c.id);reply.setCookie('session',token,{httpOnly:true,sameSite:'lax',path:'/'});return {player:{id:c.id,name:c.name}};});
}
