...
export const entities=pgTable('entities',{id:text('id').primaryKey(),name:text('name').notNull(),kind:text('kind').notNull(),mapId:text('map_id'),templateId:text('template_id'),locationId:text('location_id'),ownerId:uuid('owner_id').references(()=>characters.id),portable:boolean('portable').default(false).notNull(),openable:boolean('openable').default(false).notNull(),open:boolean('open').default(false).notNull(),replenishes:boolean('replenishes').default(false).notNull(),data:jsonb('data').$type<Record<string,unknown>>().default({}).notNull()},t=>[index('entities_map_location_idx').on(t.mapId,t.locationId),index('entities_owner_idx').on(t.ownerId)]);
